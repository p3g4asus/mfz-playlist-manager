...
function playlist_update_create_wake() {
    let $vid_row = $(`
        <div class="form-row" id="pl-update-view-wake-row" style="display: none;">
            <video loop="1" muted="1">
    ...
    `);
    $('#pl-update-view-form').append($vid_row);
    $vid_row.children('video').trigger('play');
}

function playlist_update_destroy_wake() {
    let $row = $('#pl-update-view-wake-row');
    $row.children('video').trigger('pause');
    $row.remove();
}
