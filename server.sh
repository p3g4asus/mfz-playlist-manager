...
arg="-m server --mediaset-d1 $mediaset_d1 --mediaset-d2 $mediaset_d2 --mediaset-d3 $mediaset_d3 --pid /tmp/pid_mfz_pm.pid --dbfile ../main.sqlite --static ./www --port $http_port --redis "$redis_url" --client-id "$g_id" --autoupdate $autopudate --sid $s_id --telegram $telegram --pickle "$telegram_persistence" --common-dldir "$dldir" --localfolder-basedir "$basedir" --redirect-files -v"
echo $arg
if [ "${async}" = true ]; then
    echo "Spawn!"
